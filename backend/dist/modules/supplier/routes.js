...
router.get('/', auth_js_1.authenticate, async (req, res, next) => {
    try {
        ...
        const [suppliers, total] = await Promise.all([
            database_js_1.prisma.supplier.findMany({
                where,
                orderBy: { name: 'asc' },
                skip: ((page - 1) * limit),
                take: limit,
            }),
            database_js_1.prisma.supplier.count({ where }),
        ]);
...
router.get('/:id', auth_js_1.authenticate, async (req, res, next) => {
    try {
        const supplier = await database_js_1.prisma.supplier.findUnique({ where: { id: req.params.id } });
        if (!supplier)
            throw new errorHandler_js_1.AppError(404, 'Supplier not found.', 'NOT_FOUND');
...
router.post('/', auth_js_1.authenticate, rbac_js_1.requireOperator, (0, validate_js_1.validateBody)(createSchema), async (req, res, next) => {
    try {
        const supplier = await database_js_1.prisma.supplier.create({ data: req.body });
...
router.put('/:id', auth_js_1.authenticate, rbac_js_1.requireOperator, (0, validate_js_1.validateBody)(createSchema.partial()), async (req, res, next) => {
    try {
        const supplier = await database_js_1.prisma.supplier.update({ where: { id: req.params.id }, data: req.body });
        res.json({ success: true, data: supplier });
    }
    catch (err) {
        next(err);
...
router.delete('/:id', auth_js_1.authenticate, rbac_js_1.requireOperator, async (req, res, next) => {
    try {
        await database_js_1.prisma.supplier.update({ where: { id: req.params.id }, data: { isActive: false } });
